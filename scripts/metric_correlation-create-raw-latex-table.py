...
def get_all_lang_correlations(mc, baseline, gold, system):
    ...
    lp_scores.append(ave)
    print(' & '.join(lp_scores), end=r' \\')


def write_table_content(mc, baseline, gold, system, metric, level):
...
def write_raw_table(testset, metric, level):
    ...
    write_table_content(mc, baseline, gold, system, metric, level)
