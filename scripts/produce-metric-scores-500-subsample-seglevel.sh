...
# get number of lines of human paraphrases
...
for lpnodash in deen; do
	...
	if [ ! -z $n ]; then
	    ref=`ls $para_ref_folder/$lpnodash-*.en.$num_lines | perl -pe 's/\-(\d+)\./°\1°/g' | awk -F"°" -v var="$n" '{if ($2 <= var) print}' | perl -pe 's/°/\
-/g; s/\-en\./.en./'`
	fi
	...
	scores=`$eval_tool $hyp "$num_lines" "$ref" $n`
	i=1
	    for score in $scores; do
		echo -ne "$metricname\t$lp\t$testset\t$systemname\t$i\t$score\n"
		i=$((i+1))
	    done
    done
done
