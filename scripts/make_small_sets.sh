...
basedir=/home/hltcoe/mpost/exp/parbleu19/paraphrases/2019-10-01/newstest2018
DAFILE=~/exp/parbleu19/data/wmt18/wmt18-metrics-task-package/results/out/DA-newstest2018-${lang}en-sys-nohy-scores.csv
SAMPLEMIN=1100
SAMPLEMAX=$(sacrebleu -t wmt18 -l $lang-en --echo src | wc -l)
SEEDMAX=100
...
sacrebleu -t wmt18 -l ${lang}-en --echo both | unpaste wmt18.${lang}-en.{${lang},en}
# human score
...
for samples in \$(seq $SAMPLEMIN 100 $SAMPLEMAX); do
    ...
    paste wmt18.${lang}-en.{${lang},en} | filter_lines --file sample.\$samples/\$seed/lines | unpaste sample.\$samples/\$seed/wmt18.${lang}-en.{${lang},en}

    # system outputs
    for file in ~/exp/parbleu19/data/wmt18/wmt18-submitted-data/txt/system-outputs/newstest2018/${lang}-en/newstest2018.*; do
        cat \$file | filter_lines -f sample.\$samples/\$seed/lines > sample.\$samples/\$seed/\$(basename \$file);
    done

    # paraphrase outputs
    for paraphraser in baseline-iter540000 random-1235000 treelstm-plain-545000 treelstm-semhash-560000; do
...
for sample in \$(seq $SAMPLEMIN 100 $SAMPLEMAX); do 
  ...
  for sys in sample.\$sample/\$seed/newstest2018.*; do 
    echo -ne "\$(basename \$sys | cut -d. -f 2-3)\t"; 
    cat \$sys | sacrebleu -b \$(dirname \$sys)/wmt18.$lang-en.en
...
seed=\$SGE_TASK_ID
for sample in \$(seq $SAMPLEMIN 100 $SAMPLEMAX); do
    for reftype in baseline random treelstmplain treelstmsemhash; do
        for numrefs in 1 5 10 20; do
            ...
            for sys in sample.\$sample/\$seed/newstest2018.*; do 
                echo -ne "\$(basename \$sys | cut -d. -f 2-3)\t"; 
                cat \$sys | sacrebleu -b \$(dirname \$sys)/wmt18.$lang-en.en \$refstr
