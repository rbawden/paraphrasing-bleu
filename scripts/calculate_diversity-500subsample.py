...
def read_files(para_folder, score_type=None, reference_folder=None, langpair=None, max_n=-1, secondary_para_folder=None):
    ...
    if reference_folder is not None:
        ...
        for reffile in os.listdir(reference_folder):
            if score_type == 'syntax':
                refmatch = re.match('.+?\-(....)\-ref\.en\.parse.500', reffile)
            else:
                refmatch = re.match('.+?\-(....)\-ref\.en.500', reffile)
            if not refmatch:
                continue
    ...
    filelist = [para_folder + '/' + x for x in os.listdir(para_folder) if re.match('.+?.en(\.parse)?\.500$', x)]
    filelist.extend([secondary_para_folder + '/' + x for x in os.listdir(secondary_para_folder) if re.match('.+?.en(\.parse)?\.500$', x)])
    ...
    for parafile in sorted(filelist, key=os.path.getmtime):
        ...
        matchname = re.match('.+?\/([^/]+en)\-(\d+)\.en(\.parse)?\.500', parafile)
        if not matchname:
            continue
