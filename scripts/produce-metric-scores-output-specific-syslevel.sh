...
if [[ "$#" -ne 3 ]]; then
    ...
    echo -e "\tEVALTOOL: scripts/sacreBLEU-syslevel or scripts/Meteor-syslevel"
    exit
fi
...
for lpnodash in `ls $refdir/$testset/*-ref.en | perl -pe 's/^.+\-([^-]+?)\-ref\.en$/\1/'`; do
	...
	#echo "$hyp $ref $para"
	score=`$eval_tool $hyp "$ref $para" 1`
	echo -ne "$modeltype\t$lp\t$testset\t$systemname\t$score\n"
    done
done
