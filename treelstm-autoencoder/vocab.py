...
class Vocab(object):
    def __init__(self, vocab_file=None):
        ...
        # specifical tokens for padding, end-of-sentence and unknown words
        self.pad_sym = "<pad>"
        self.eos_sym = "<eos>"
        self.unk_sym = "<unk>"
...
if __name__ == "__main__":
    ...
    parser.add_argument('--tree', action='store_true', help='whether input tree structure')
    ...
    with open(args.input, 'r') as reader:
        for line in reader:
            if args.tree:
                parse_tree = tree.string_to_tree(line.strip(), keep_leaf=False)
                for token in tree.get_labels_on_tree(parse_tree):
                    vocab.insert(token)
            else:
